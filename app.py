...
def create_top_10_by_exchange_tables():
    try:
        df = pd.read_csv('top_10_local_data.csv') # Hoặc dùng đường dẫn đầy đủ như đã hướng dẫn
    except FileNotFoundError:
        return html.Div("")

    exchanges = ['HOSE', 'HNX', 'UPCOM']
    list_of_tabs = [] # Thay vì list_of_tables, ta tạo list_of_tabs

    for ex in exchanges:
        exchange_df = df[df['Sàn'] == ex].copy()
        top_10 = exchange_df.sort_values(by="Giá trị GD", ascending=False).head(10)
        
        # Tạo bảng DataTable như cũ
        ...
        # TẠO TAB: Bọc table_component trong một dcc.Tab
        # Mỗi tab có 'label' (tên hiển thị) và 'children' (nội dung bên trong)
        tab = dcc.Tab(label=ex, children=[
            html.Div(table_component, style={'marginTop': '10px'}) # Thêm khoảng đệm cho đẹp
        ])
        
        list_of_tabs.append(tab)

    # TRẢ VỀ KẾT QUẢ: Bọc danh sách các tab trong dcc.Tabs
    return html.Div([
...
app.layout = html.Div([
    ...
    html.H1("Thị trường chứng khoán Việt Nam", style={'textAlign': 'center'}),
    ...
    # --- KHU VỰC CHÍNH: BẢN ĐỒ VÀ BẢNG TOP 10 ---
    html.Div([
        
        # --- CỘT BÊN TRÁI: BẢN ĐỒ THỊ TRƯỜNG (TREEMAP) ---
        html.Div([
            html.H2("Bản đồ thị trường", style={'textAlign': 'center', 'marginTop': '30px'}),
            html.Div([
                dcc.Dropdown(
                    id='dropdown-metric',
                    options=[
                        {'label': 'Vốn hóa thị trường', 'value': 'market_cap'},
                        {'label': 'Giá trị giao dịch', 'value': 'trading_value'} # Sửa lại value cho đúng với master_data
                    ],
                    value='market_cap',
        ...
        # --- CỘT BÊN PHẢI: BẢNG TOP 10 ---
        html.Div([
            # Bảng Top 10 theo Sàn
            create_top_10_by_exchange_tables(), 
            
            # ✅ THÊM BẢNG TOP 10 TĂNG GIÁ ĐÃ SỬA ĐỔI
            create_dynamic_top_10_table(df_master, metric_type='price_volatility', period='day')
        ], style={'width': '30%', 'display': 'inline-block', 'verticalAlign': 'top'})
    ]), 
    # --- KHU VỰC BIỂU ĐỒ NẾN (MẶC ĐỊNH SẼ ẨN ĐI) ---
    html.Div(
        ...
        children=[
            ...
            dcc.RadioItems(
                ...
                options=[{'label': '1M', 'value': '1M'}, {'label': '3M', 'value': '3M'}, 
...
def update_treemap_callback(selected_metric, selected_sector):
    ...
    if np.isnan(max_abs_change) or max_abs_change == 0:
        max_abs_change = 0.01  # tránh chia cho 0 hoặc all NaN
    ...
    # ✅ Áp dụng màu thủ công cho từng ô
        ...
        for label, val, parent in zip(labels, trace.marker.colors, parents):
            if label in all_parents or label == "All":
                ...
                if val is None or (isinstance(val, float) and np.isnan(val)):
                    # ô không có dữ liệu => trắng (trung tính)
                    new_colors.append("#FFFFFF")
                else:
                    # chuẩn hóa giá trị để xác định màu
                    v = float(val)
...
def activate_and_update_stock_chart(search_symbol, click_data):
    ...
    df_full = df_full.iloc[:-1]  # Đảo ngược thứ tự nếu cần
