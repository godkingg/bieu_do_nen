...
def get_market_info_yahoo(symbols: list):
    ...
    try:
        ...
        # GỢI Ý MỚI: Lấy dữ liệu từ cả hai nguồn summary_detail và price
        ...
        for item in data_list:
            ...
            market_cap = None
            # Ưu tiên tìm trong summary_info trước
            if isinstance(summary_info, dict):
                market_cap = summary_info.get('marketCap')
            
            # GỢI Ý MỚI: Nếu không tìm thấy, thử tìm trong price_info (phương án dự phòng)
...
# Thay thế hàm cũ bằng hàm đã cập nhật này
...
def create_master_data_intelligent_update():
    ...
    if os.path.exists(output_file):
        try:
            ...
            print(f"Phân tích file: {len(outdated_symbols)} mã cần cập nhật, {len(new_symbols)} mã mới.")
    ...
    if not symbols_to_process:
        print("Tất cả dữ liệu đã được cập nhật. Hoàn tất!")
        return pd.read_csv(output_file) if os.path.exists(output_file) else pd.DataFrame()
        
    print(f"Tổng cộng có {len(symbols_to_process)} mã cần xử lý... Sẽ lưu file mỗi {save_interval} mã.")
        ...
        for i, symbol in enumerate(tqdm(symbols_to_process, desc="Processing symbols")):
            ...
            try:
                ...
                if df_history is not None and len(df_history) >= 2:
                    df_history_reset = df_history.reset_index(drop=True)
                    latest_rows = df_history_reset[df_history_reset['Date'] == latest_trading_date]
                    if not latest_rows.empty:
                        latest_index = latest_rows.index[0]
                        if latest_index > 0:
                            latest = df_history_reset.iloc[latest_index]
                            previous = df_history_reset.iloc[latest_index - 1]
                            
                            # Kiểm tra dữ liệu trước khi tính toán để tránh lỗi như của mã DVT
                            if pd.notna(latest['Close']) and pd.notna(previous['Close']) and pd.notna(latest['Volume']) and pd.notna(previous['Volume']):
                                price_change_pct = (latest['Close'] - previous['Close']) / previous['Close'] if previous['Close'] > 0 else 0
                                volume_change_pct = (latest['Volume'] - previous['Volume']) / previous['Volume'] if previous['Volume'] > 0 else 0
                                latest_trading_value = latest['Close'] * latest['Volume']
                                previous_trading_value = previous['Close'] * previous['Volume']
                                trading_value_change_pct = (latest_trading_value - previous_trading_value) / previous_trading_value if previous_trading_value > 0 else 0
                                
                                batch_data.append({
                                    'symbol': symbol, 'date': latest['Date'], 'close': latest['Close'],
                                    'volume': latest['Volume'], 'price_change_pct': price_change_pct,
            ...
            if (i + 1) % save_interval == 0 or (i + 1) == len(symbols_to_process):
                # Bây giờ bạn sẽ thấy các lệnh debug bên dưới nếu batch_data có dữ liệu
                if not batch_data:
...
def read_stock_data_for_date(stock_info, target_date, data_dir="data"):
    ...
    try:
        ...
        # Lọc chính xác ngày mong muốn
        daily_data = df[df['Date'] == target_date]
        
        if not daily_data.empty:
            record = daily_data.iloc[0]
            # Tính toán giá trị giao dịch từ Cột 'Close' và 'Volume' đã được xử lý
            ...
            return {
                ...
                # Lấy giá đóng cửa trực tiếp từ cột 'Close' đã chuẩn hóa
...
def generate_top_10_component_from_local():
    ...
    """
    # 1. Xác định ngày giao dịch mới nhất
    ...
    # 2. Lấy danh sách tất cả các mã chứng khoán
    all_stocks_df = get_stock_map()
    stock_list = all_stocks_df.to_dict('records')
    
    all_daily_data = []

    # 3. Sử dụng ThreadPoolExecutor để đọc song song nhiều file, tăng tốc độ
